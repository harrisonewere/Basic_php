...
$name_error = $email_error = $password_error = $image_error ="";
  ...
  function uploadFile($files, $allowed_types =[], &$error_message = '')
  {
    $target_dir ='uploads/';
    $target_file = $target_dir .rand() . basename($files['name']);
  ...
  if(!empty($password) &&strlen($password)<6){
    $password_error = "Password cannot be less than 6 (six) characters";
  }
